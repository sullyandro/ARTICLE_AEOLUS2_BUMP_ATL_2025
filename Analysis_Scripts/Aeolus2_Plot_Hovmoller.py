...
for exp in cases:
	...
	u1_t    = dat.variables['u1ph'][:] # Zonal (azimuthal) velocity layer 1           ; units: [(g*H)^0.5], [beta*L_d^2] at the equator
	v1_t    = dat.variables['u1th'][:] # Meridional velocity layer 1                  ; units: [(g*H)^0.5], [beta*L_d^2] at the equator

	u2_t    = dat.variables['u2ph'][:] # Zonal (azimuthal) velocity layer 2           ; units: [(g*H)^0.5], [beta*L_d^2] at the equator
	v2_t    = dat.variables['u2th'][:] # Meridional velocity layer 2                  ; units: [(g*H)^0.5], [beta*L_d^2] at the equator

	h1_t    = dat.variables['h1'  ][:] # Pseudo-height layer 1                        ; units: H
	h2_t    = dat.variables['h2'  ][:] # Pseudo-height layer 2                        ; units: H

	b1_t    = dat.variables['b1'  ][:] # Buoyancy layer 1                             ; units: g*theta/theta_s
	b2_t    = dat.variables['b2'  ][:] # Buoyancy layer 2                             ; units: g*theta/theta_s

	cc1_t   = dat.variables['CC1' ][:] # CLWC (Cloud Liquid Water Content) at layer 1 ; units: Q/T
	w2_t    = dat.variables['w2'  ][:] # Bulk of Precipitable Water at layer 2        ; units: (L_v.g/(C_p.theta_s))Kg/Kg
